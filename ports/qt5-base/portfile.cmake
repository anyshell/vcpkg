...
if(VCPKG_TARGET_IS_FREEBSD)
    # qt5 freebsd build bug: https://bugreports.qt.io/browse/QTBUG-65425?page=com.atlassian.jira.plugin.system.issuetabpanels%3Aall-tabpanel
    list(APPEND PATCHES patches/freebsd.patch)
endif()
...
elseif(VCPKG_TARGET_IS_LINUX OR VCPKG_TARGET_IS_FREEBSD OR VCPKG_TARGET_IS_OPENBSD)
    ...
    elseif(${TARGET_TRIPLET} MATCHES "freebsd" OR ${TARGET_TRIPLET} MATCHES "openbsd")
        list(APPEND CORE_OPTIONS -no-fontconfig -no-xcb-xlib -no-xcb)

        # configure options from here: https://svnweb.freebsd.org/ports/head/devel/qt5-core/Makefile?view=markup
        #list(APPEND CORE_OPTIONS -no-accessibility -no-gif -no-libpng -no-libjpeg
        #                         -no-gui -no-widgets -no-cups -no-iconv -no-dbus -no-opengl
        #                         -no-egl -no-evdev -no-fontconfig -no-freetype -no-gtk -no-harfbuzz
        #                         -no-libudev -no-xcb -no-xcb-xlib -no-xkbcommon -no-libinput)
    else()
        list(APPEND CORE_OPTIONS -no-fontconfig -xcb-xlib -xcb -linuxfb)
    endif()
    if (VCPKG_TARGET_IS_LINUX AND NOT EXISTS "/usr/include/GL/glu.h")
